...
export class DocumentService {
  ...
  async saveDocument(userId: string, file: Express.Multer.File) {
    ...
    const fixedPath = file.path.endsWith(ext)
      ? file.path
      : `${file.path}${ext}`;
  ...
  async processOcr(documentId: string, userId: string) {
    ...
    if (ext === '.pdf') {
      text =
        (await extractTextFromPdf(document.filepath)) ||
        (await runOcrOnPdf(document.filepath));
    } else {
  ...
  async askQuestion(documentId: string, userId: string, question: string) {
    ...
    if (!doc?.ocrResult) {
      throw new NotFoundException(
        'Documento não encontrado ou sem resultado de OCR',
      );
    }
